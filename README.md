...
- `npm run prove:minutes-votes -- --queries=carried,no,failed,unanimous --maxItems=8 [--probeOutput=/tmp/probe.json] [--live] [--persist]` — default to the checked-in `server/fixtures/minutes-votes-probe-output.json` capture for deterministic proof runs, or opt back into live discovery with `--live`; either way, bucket vote patterns, surface showcase cases plus remaining failure modes, and optionally persist only persist-ready samples when `DATABASE_URL` is available
...
- Minutes-backed search hits (`SourceType = MinutesSearch`, `ExistInMinutes = true`) can also be opened through `GetCompleteAgendaItem_V1`; minutes/result text lives under `Minutes.Minutes` and `Minutes.VotingHTML`, while adjacent `GetMeetingItemDetails` calls can still return `204`.
